...
import { RecentSearchService, THE_FI_COMPANY_RECENT_SEARCH } from '../services/recent-search.service';
...
export class FilterPeoplePipe implements PipeTransform {
    ...
    transform(value: any, query: string, field: string): any {
        return query ? value?.reduce((prev, next) => {
            if (
                next[field]?.toLocaleLowerCase().includes(query?.toLocaleLowerCase()) ||
                next['description']?.toLocaleLowerCase().includes(query?.toLocaleLowerCase()) ||
                next['label']?.toLocaleLowerCase().includes(query?.toLocaleLowerCase())
            ) {
                prev.push(next);
            }
            return prev;
...
export class SearchModalService {
    ...
    open() {
        if (!this.modalService.hasOpenModals()) {
            return this.modalService.open(SearchModalComponent, { size: 'lg', keyboard: true, backdrop: true });
        } return null;
    }
...
export class SearchModalComponent {
    ...
    setRecentSearch(item) {
        ...
        localStorage.setItem(THE_FI_COMPANY_RECENT_SEARCH, JSON.stringify(this.recentSearches));
    }
