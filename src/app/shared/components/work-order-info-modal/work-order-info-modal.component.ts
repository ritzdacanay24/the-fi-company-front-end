...
export class WorkOrderInfoModalService {
  ...
  public open(workOrderNumber: number) {
    this.modalRef = this.modalService.open(WorkOrderInfoModalComponent, {
      size: "xl",
      fullscreen: false,
      backdrop: "static",
      scrollable: true,
...
export class WorkOrderInfoModalComponent {
  ...
  columnDefs: any = [
    ...
    },
    {
      ...
      headerName: "% Complete",
      filter: "agTextColumnFilter",
      valueFormatter: (params) => params.data.LINESTATUS.toFixed(2),
      cellClass: (params) => {
  ...
  getData = async () => {
    try {
      this.isLoading = true;
      let data = await this.workOrderInfoService.getDataByWorkOrderNumber(
