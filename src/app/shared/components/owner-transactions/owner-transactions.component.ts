...
export class OwnerTransactionsService {
  ...
  open(soNumberAndLineNumber) {
    this.modalRef = this.modalService.open(OwnerTransactionsComponent, { size: 'lg' });
    this.modalRef.componentInstance.soNumberAndLineNumber = soNumberAndLineNumber;
    this.modalRef.componentInstance.passEntry.subscribe((receivedEntry: any) => this.modalRef.close())
...
export class OwnerTransactionsComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.soNumberAndLineNumber)
      this.getData(this.soNumberAndLineNumber);
  }
