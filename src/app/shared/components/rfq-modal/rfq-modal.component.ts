...
export class RfqModalService {
  ...
  open(so: string, line) {
    this.modalRef = this.modalService.open(RfqModalComponent, { size: 'lg', fullscreen: false, backdrop: 'static', scrollable: true, centered: true, keyboard: false });
...
export class RfqModalComponent {
  ...
  getData = () => {
    this.isLoading = true;
    this.api.searchBySoAndSoLine(this.so, this.line).pipe(first()).subscribe(data => {
