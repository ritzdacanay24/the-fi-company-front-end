...
export class KanbanQueueModalService {
  ...
  open(data: string) {
    this.modalRef = this.modalService.open(
      WorkOrderTrackerQueueModalComponent,
      { size: "md" }
    );
...
export class WorkOrderTrackerQueueModalComponent {
  ...
  async onSubmit() {
    ...
    try {
      ...
      let e: any = await this.checkIfPickComplete(
        this.data.wo_nbr,
        this.currentSelectionRouting,
        this.data.kanban_ID
      );
      if (e?.errorMessage) {
  ...
  async checkIfPickComplete(wo_nbr, route, queue) {
    return await this.kanbanApiService.checkIfPickComplete(
      wo_nbr,
      route,
      queue
    );
