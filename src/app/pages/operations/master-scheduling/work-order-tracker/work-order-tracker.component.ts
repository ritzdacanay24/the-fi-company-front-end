...
export class FilterlistPipe implements PipeTransform {
  transform(value: any, args?: any): any {
    if (!args || args.indexOf("All") > -1) return value;
...
export class MyFilterPipe implements PipeTransform {
  transform(items: any[], filter: any): any {
    ...
    return items.filter((item) => {
      return (
        item?.wo_nbr?.toString().indexOf(filter?.toString()) !== -1 ||
...
export class WorkOrderTrackerComponent implements OnInit {
    ...
    this.websocketService.messageSubject$?.subscribe((data) => {
      if (data && data.type == "close") {
        this.liveUpdateMessage =
          "This page is no longer receiving live updates. Please refresh.";
      } else {
        this.liveUpdateMessage = "";
      }
    });
    ...
    if (name == "Over Due") {
      ...
      for (let i = 0; i < this.data.queues.length; i++) {
        for (let ii = 0; ii < this.data.queues[i].details.length; ii++) {
          let row = this.data.queues[i].details[ii];
          let pickingPercent =
            (row?.pickInfo?.wod_qty_iss / row?.pickInfo?.wod_qty_req) * 100;
  ...
  ngOnInit(): void {
    this.activatedRoute.queryParams.subscribe((params) => {
      ...
      this.showLess =
        params["showLess"] == "false" || !params["showLess"] ? false : true;
    });
  ...
  openQueueSelection(tasks, header) {
    if (
      header?.user_roles?.indexOf(
        this.authenticationService.currentUserValue.full_name
      ) === -1
    ) {
      this.toastrService.warning(
        "You dont have access to move orders in this queue."
      );
      return;
    }
