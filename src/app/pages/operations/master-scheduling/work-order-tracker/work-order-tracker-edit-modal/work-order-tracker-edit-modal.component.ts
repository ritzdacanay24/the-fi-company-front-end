...
export class KanbanEditModalService {
  ...
  open(id: string) {
    this.modalRef = this.modalService.open(WorkOrderTrackerEditModalComponent, {
      size: "lg",
    });
...
export class WorkOrderTrackerEditModalComponent {
  ...
  async onSubmit() {
    //this.isLoading = true;
    try {
      this.form.value.start_time = this.form.value.start_time
        ? this.form.value.start_time
        : null;
      this.form.value.end_time = this.form.value.end_time
