...
export class CalendarComponent implements OnInit {
  ...
  async getData(firstDay, lastDay, clearData = true) {
    ...
    let data: any = await this.achedulerService.getData(moment(firstDay).format('YYYY-MM-DD'), moment(lastDay).format('YYYY-MM-DD'));
    const newEvents = [];
  ...
  getAttribut() {
    setTimeout(() => {
      const el1: any = document.querySelector(`[ng-reflect-id="${this.previousId}"]`);
