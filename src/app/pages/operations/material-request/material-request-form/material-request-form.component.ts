...
export class MaterialRequestFormComponent {
  ...
  async onValidate() {
    ...
    try {
      ...
      let data = await this.materialRequestService
        .searchByItem(this.listItems);
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        this.details.push(this.fb.group({
          "partNumber": new FormControl(row.partNumber || null, Validators.required),
          "qty": new FormControl(row.qty, Validators.required),
          "isDuplicate": isDup,
          "reasonCode": new FormControl(row.reasonCode || null, Validators.required),
          "message": row.message,
        ...
        if (data[i]?.hasError || isDup) {
          this.details.controls[i]['controls']['partNumber'].setErrors({ 'invalid': true })
        }
