...
export class SerialNumberReportComponent implements OnInit {
  ...
  private readonly USE_MOCK_DATA = false; // Set to false to use real API
  ...
  constructor(
    ...
    // Initialize add serial form (keeping for compatibility, but redirecting to standalone)
  ...
  loadSerialNumbers() {
    if (this.USE_MOCK_DATA) {
      ...
      // Real API call
      this.serialNumberService.getSerialHistory(1000, this.selectedTemplate, this.selectedStatus).subscribe({
        next: (response) => {
          // Handle both direct array response and wrapped response
          if (Array.isArray(response)) {
            this.serialNumbers = response;
            console.log('Serial numbers loaded:', this.serialNumbers.length);
          } else if (response.success && response.data) {
            this.serialNumbers = response.data;
            console.log('Serial numbers loaded:', this.serialNumbers.length);
          } else if (response.success === false) {
            this.toastrService.error('Failed to load serial numbers: ' + (response.message || response.error));
            this.serialNumbers = [];
          } else {
            console.error('Unexpected response format:', response);
            this.toastrService.error('Unexpected response format from server');
            this.serialNumbers = [];
          }
        },
        error: (error) => {
  ...
  // Modal methods - Redirects to standalone generator instead of showing modal
  open(content: TemplateRef<any>) {
    // Redirect to standalone serial number generator
    this.router.navigate(['/standalone/serial-generator']);
    this.toastrService.info('Redirecting to Serial Number Generator...', 'Navigation');
  }

  // Keep for navigation to standalone generator
  navigateToGenerator() {
    this.router.navigate(['/standalone/serial-generator']);
  }
