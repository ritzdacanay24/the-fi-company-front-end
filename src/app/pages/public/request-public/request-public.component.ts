...
export class RequestPublicComponent implements OnInit {
  ...
  async onSubmit() {
    ...
    try {
      ...
      SweetAlert.fire({ text: `Request submitted successfully. Your request ID # is ${data.id}. ` })
    } catch (err) {
  ...
  async getAttachments() {
    this.attachments = await this.attachmentsService.getAttachmentByRequestId(this.request_id);
  }
  ...
  async getData() {
    try {
      this.isLoading = true;
      let data: any = this.data = await this.requestService.getByToken(this.token);
      this.request_id = data.id;
      this.comments = await this.commentsService.getByRequestId(this.request_id);
      this.jobInfo = await this.requestService.getjobByRequestId(this.request_id);
