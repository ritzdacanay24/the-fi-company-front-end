...
@Component({
  standalone: true,
  imports: [SharedModule, MbscModule, JobOverviewComponent, TicketOverviewComponent, JobEditComponent, JobSearchComponent],
...
export class CalendarComponent implements OnInit {
  ...
  async getData(firstDay, lastDay, clearData = true) {
    ...
    try {
      this.isLoading = true;
      let data: any = this.data = await this.achedulerService.fsCalendar(moment(firstDay).format('YYYY-MM-DD'), moment(lastDay).format('YYYY-MM-DD'));
      const newEvents = [];
