...
const url = 'apiV2/computer';
const legacyUrl = 'operations/computer';

@Injectable({
  providedIn: 'root',
})
export class ComputerService extends DataService<any> {
  ...
  getMaintenanceHistory = async (computerId: number): Promise<any[]> => {
    return await firstValueFrom(this.http.get<any[]>(`${url}/maintenance?computer_id=${computerId}`));
  };
