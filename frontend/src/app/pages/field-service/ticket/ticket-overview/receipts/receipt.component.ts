...
export class ReceiptComponent implements OnInit {
  ...
  async getData(clear = true) {
    ...
    try {
      this.loading = true;
      this.data = await this.tripExpenseService.getByWorkOrderId(
        this.workOrderId
      );
    ...
    for (let i = 0; i < this.transactions.length; i++) {
      ...
      for (let ii = 0; ii < this.data.length; ii++) {
        if (
          ...
          transactions[i].possibleCount == 1
        ) {
  ...
  columnDefs: ColDef[] = [
...

    {
      ...
      cellEditorParams: {
        values: ["Lost Receipt", "Unknown", "Damaged", "NA"],
        cellEditorPopup: false,
      },
      valueGetter: (params) => {
        if (params.data.reason_code != "NA") return params.data.reason_code;
        return "";
      },
    },
    {
