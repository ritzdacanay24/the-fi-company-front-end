...
@Component({
  selector: "app-request-public-v2-horizontal",
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, NgSelectModule, AddressSearchComponent],
  templateUrl: "./request-public-v2.horizontal-preview.html",
  styleUrls: ["./request-public-v2.component.scss"],
})
export class RequestPublicV2HorizontalComponent {
  submitted = false;
  submitting = false;
  ...
  private createProductGroup(): FormGroup {
    return this.fb.group({
      ...
      sign_theme: ["", Validators.required],
      sign_mount: ["", Validators.required],
      eyefi_customer_part_number: ["", Validators.required],
      sn: ["", Validators.required],
      customer_product_number: [null, Validators.required],
  ...
  async onSubmit(): Promise<void> {
    this.submitted = true;
    if (this.form.invalid || this.hasMissingRequiredProductFiles()) {
  ...
  isProductFilesInvalid(requestIndex: number, productIndex: number): boolean {
    return this.submitted && this.getProductFiles(requestIndex, productIndex).length === 0;
  }
  ...
  private hasMissingRequiredProductFiles(): boolean {
    for (let requestIndex = 0; requestIndex < this.serviceRequests.length; requestIndex++) {
      const products = this.getProducts(requestIndex);
      for (let productIndex = 0; productIndex < products.length; productIndex++) {
        if (this.getProductFiles(requestIndex, productIndex).length === 0) {
          return true;
        }
      }
    }
  ...
  private expandInvalidSections(): void {
    for (let requestIndex = 0; requestIndex < this.serviceRequests.length; requestIndex++) {
      const requestGroup = this.serviceRequests.at(requestIndex);
      const products = this.getProducts(requestIndex);
      const hasInvalidProduct = products.controls.some((productGroup, productIndex) => {
        return productGroup.invalid || this.getProductFiles(requestIndex, productIndex).length === 0;
      });

      if (requestGroup.invalid || hasInvalidProduct) {
        this.serviceRequestExpanded[requestIndex] = true;
      }
      ...
      products.controls.forEach((productGroup, productIndex) => {
        if (productGroup.invalid || this.getProductFiles(requestIndex, productIndex).length === 0) {
          this.productExpanded[requestIndex][productIndex] = true;
        }
      });
    }
  }
  ...
  private focusFirstInvalidControl(): void {
    setTimeout(() => {
      const element = document.querySelector(".is-invalid, .file-required-invalid") as HTMLElement | null;
      element?.focus();
