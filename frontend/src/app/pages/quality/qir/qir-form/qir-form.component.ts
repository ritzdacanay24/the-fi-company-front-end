...
export class QirFormComponent {
  ...
  ngOnInit(): void {
    ...
    this.form.get("type1").valueChanges.subscribe((mode: string) => {
    ...
    this.form.get("stakeholder").valueChanges.subscribe((mode: string) => {
  ...
  async getQirSettings() {
    let formSettings: any = await this.qirSettingsService.getFormSettings({
      active: 1,
    });
  ...
  notifyParent($event) {
    this.form.patchValue(
      { fieldServiceSchedulerId: $event.id },
      { emitEvent: false }
    );
  }
