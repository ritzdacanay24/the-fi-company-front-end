...
export class MrbFormComponent {
  ...
  calculate() {
    this.form.get('itemCost').valueChanges.subscribe(
      (mode: any) => {
        this.setValue('value', (this.form.get('itemCost').value * this.form.get('qtyRejected').value).toFixed(2))
      });
    this.form.get('qtyRejected').valueChanges.subscribe(
      (mode: any) => {
        mode && this.setValue('value', (this.form.get('itemCost').value * this.form.get('qtyRejected').value).toFixed(2))
      });
  }
