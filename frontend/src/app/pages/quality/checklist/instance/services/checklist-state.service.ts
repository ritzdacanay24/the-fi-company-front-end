...
export interface ChecklistItemProgress {
  ...
  completed: boolean;
  photos: string[];
  photoMeta?: Record<string, { source?: 'in-app' | 'system' | 'library'; uploadedAt?: string; uploadedBy?: string; uploadedByUserId?: number; mediaId?: number }>; // per-photo metadata by URL
  videos?: string[]; // NEW: Array of uploaded video URLs
  videoMeta?: Record<string, { source?: 'in-app' | 'system' | 'library'; uploadedAt?: string; uploadedBy?: string; uploadedByUserId?: number; mediaId?: number }>; // per-video metadata by URL
  notes: string;
  completedAt?: Date;
...
interface CompletionData {
  ...
  photoMeta?: Record<string, { source?: 'in-app' | 'system' | 'library'; uploadedAt?: string; uploadedBy?: string; uploadedByUserId?: number; mediaId?: number }>;
  videoMeta?: Record<string, { source?: 'in-app' | 'system' | 'library'; uploadedAt?: string; uploadedBy?: string; uploadedByUserId?: number; mediaId?: number }>;
...
export class ChecklistStateService {
  ...
  addVideo(
    ...
    const videoMeta: Record<string, { source?: 'in-app' | 'system' | 'library'; uploadedAt?: string; uploadedBy?: string; uploadedByUserId?: number; mediaId?: number }> = {};
    videoMeta[videoUrl] = {
      uploadedAt: new Date().toISOString(),
      uploadedBy: userName,
      uploadedByUserId: userId,
