...
export class SerialNumberManagerComponent implements OnInit {
  ...
  async generateRange(): Promise<void> {
    ...
    try {
      ...
      const createdCount = result?.created ?? serialNumbers.length;
      this.toastrService.success(`Successfully generated ${createdCount} serial numbers`);
