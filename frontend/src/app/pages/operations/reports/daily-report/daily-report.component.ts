...
import { DailyReportHistoryService } from '../daily-report-history/daily-report-history.service';
...
export class DailyReportComponent implements OnInit {
    ...
    constructor(
        ...
        private dailyReportHistoryService: DailyReportHistoryService
    ) { }
    ...
    async setHistoryMode(): Promise<void> {
        ...
        this.selectedDate = moment().subtract(1, 'day').format('YYYY-MM-DD');
        this.modeMessage = 'Showing historical snapshot from the History API.';
        await this.loadHistoryData(this.selectedDate);
    }
    ...
    async navigateHistoryDate(dayDelta: number): Promise<void> {
        const nextDate = moment(this.selectedDate).add(dayDelta, 'day').format('YYYY-MM-DD');
        await this.onHistoryDateChange(nextDate);
    }
    ...
    private async loadHistoryData(date: string): Promise<void> {
        try {
            this.isLoading = true;

            const response: any = await firstValueFrom(this.dailyReportHistoryService.getDailyReportHistory({
                startDate: date,
                endDate: date,
                page: 1,
                limit: 1
