...
export class OtdReportComponent implements OnInit {
  ...
  columnDefs: ColDef[] = [
    ...
    {
      field: "last_shipped_on",
      headerName: "Last Shipped On",
      filter: "agTextColumnFilter",
    },
    {
      field: "shipped_partial",
      headerName: "Shipped Partial",
      filter: "agTextColumnFilter",
    },
    ...
    { field: "total_lines", headerName: "Total", filter: "agTextColumnFilter" },
    {
      ...
      cellClass: (params: any) => {
        if (params.data) {
          ...
          if (params.value < this.goal) return ["bg-danger-subtle text-danger"];
          return params.value;
        }
        return null;
      },
  ...
  totalRow = function (api) {
    ...
    this.calcTotalCols.forEach((params) => {
      ...
      if (params == "value") {
        result[0]["value"] =
          result[0]["total_lines"] > 0
            ? (result[0]["total_shipped_on_time"] / result[0]["total_lines"]) *
              100
            : 0;
      }
    });
  ...
  get getValue() {
    return (
      ((this.allInfo?.todayInfo?.value - this.allInfo?.yesterdayInfo?.value) /
        this.allInfo?.yesterdayInfo?.value) *
      100
    );
  }
  ...
  async getData() {
    try {
      this.gridApi?.showLoadingOverlay();
      this.isLoading = true;
      let data: any = await this.reportService.getOtdReportV1(
        this.dateFrom,
        this.dateTo,
        this.displayCustomers,
        this.typeOfView
