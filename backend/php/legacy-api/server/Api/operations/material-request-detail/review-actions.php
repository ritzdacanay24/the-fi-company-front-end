...
require_once __DIR__ . '/../../../../shared/EmailTemplate.php';
require_once __DIR__ . '/../../../../shared/ReviewEmailService.php';
...
function handleBulkAssign($pdo, $input) {
    ...
    try {
        ...
        $emailResults = [];
        try {
            $emailService = new ReviewEmailService($pdo);
            $emailResults = $emailService->sendBulkAssignmentEmails($created_reviews);
        } catch (Exception $emailException) {
            // Log email error but don't fail the entire operation
            error_log("Email notification failed: " . $emailException->getMessage());
            $emailResults = [
                'success' => false,
                'message' => 'Review assignments completed but email notifications failed: ' . $emailException->getMessage()
            ];
        }
...
function handleBulkReview($pdo, $input) {
    ...
    try {
        ...
        try {
            $emailService = new ReviewEmailService($pdo);
            $emailResults = $emailService->sendBulkReviewEmails($updated_reviews, $decision);
        } catch (Exception $emailException) {
            // Log email error but don't fail the entire operation
            error_log("Email notification failed: " . $emailException->getMessage());
            $emailResults = [
                'success' => false,
                'message' => 'Review completed but email notifications failed: ' . $emailException->getMessage()
            ];
        }
...
function handleDepartmentSummary($pdo, $input) {
    ...
    $stmt = $pdo->prepare("
        SELECT 
            ...
            GROUP_CONCAT(DISTINCT CONCAT(u.first, ' ', u.last)) as reviewers,
        ...
        LEFT JOIN db.users u ON r.reviewerId = u.id
...
function handleEscalateReview($pdo, $input) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT r.*, md.partNumber, md.description, md.qty,
                   CONCAT(original_reviewer.first, ' ', original_reviewer.last) as original_reviewer_name,
                   CONCAT(escalator.first, ' ', escalator.last) as escalated_by_name,
                   CONCAT(requestor.first, ' ', requestor.last) as requested_by_name
            FROM mrf_det_reviews r
            INNER JOIN mrf_det md ON r.mrf_det_id = md.id
            LEFT JOIN db.users original_reviewer ON r.reviewerId = original_reviewer.id
            LEFT JOIN db.users escalator ON ? = escalator.id
            LEFT JOIN db.users requestor ON r.sentForReviewBy = requestor.id
            WHERE r.id = ? AND r.active = 1
        ");
        $stmt->execute([$escalated_by, $review_id]);
        ...
        try {
            $emailService = new ReviewEmailService($pdo);
            $escalationData = [
                'original_reviewer' => $original_review['original_reviewer_name'],
                'escalated_by' => $original_review['escalated_by_name'],
                'escalation_reason' => $escalation_reason,
                'partNumber' => $original_review['partNumber'],
                'description' => $original_review['description'],
                'qty' => $original_review['qty'],
                'department' => $original_review['department'],
                'requested_by' => $original_review['requested_by_name'],
                'request_date' => $original_review['sentForReviewAt'],
                'original_review_notes' => $original_review['reviewNote']
            ];
            
            $emailResult = $emailService->sendEscalationEmail($new_reviewer_id, $escalationData);
        } catch (Exception $emailException) {
            // Log email error but don't fail the entire operation
            error_log("Escalation email notification failed: " . $emailException->getMessage());
            $emailResult = [
                'success' => false,
                'message' => 'Escalation completed but email notification failed: ' . $emailException->getMessage()
            ];
        }
...
function handleRequestClarification($pdo, $input) {
    ...
    try {
        ...
        try {
            $emailService = new ReviewEmailService($pdo);
            $emailResult = $emailService->sendClarificationEmail($review_id, $clarification_request);
        } catch (Exception $emailException) {
            // Log email error but don't fail the entire operation
            error_log("Clarification email notification failed: " . $emailException->getMessage());
            $emailResult = [
                'success' => false,
                'message' => 'Clarification request submitted but email notification failed: ' . $emailException->getMessage()
            ];
        }
