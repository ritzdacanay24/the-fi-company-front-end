...
namespace EyefiDb\Api\Graphics\GraphicsOrderView;
use PDO; 
use PDOException;	

class GraphicsOrderView
{
	...
	public function ReadSingle($co)
	{
		...
		while ($row = $stmt->fetch()) {
			...
			$obj['queues'][] = array(
				"id"					=> $row['id'], "name"				=> $row['name'], "path"				=> $row['path'], "queueStatus"			=> $row['queueStatus'], "classColor"			=> $classColor, "shipStatus"			=> $shipStatus
			);
		}
		...
		while ($row1 = $stmt1->fetch(PDO::FETCH_ASSOC)) {
			...
			$obj['trans'][] = array(
				"id" 					=> $row1['id'], "field" 			=> $row1['field'], "o" 					=> $row1['o'], "n" 					=> $row1['n'], "createDate" 	=> $row1['createDate'], "comment" 	=> $row1['comment'], "userId" 			=> $row1['userId'], "so" 				=> $row1['so'], "type" 			=> $row1['type'], "userName" 	=> $row1['userName']
			);
		}
