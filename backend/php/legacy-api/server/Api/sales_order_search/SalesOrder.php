...
namespace EyefiDb\Api\sales_order_search;
	...
	public function Read($order)
	{
		...
		AND a.active = 1
			...
			foreach ($mainDetails1 as $row) {
				...
				$query->bindParam(':so_nbr', $soLine, PDO::PARAM_STR);
				$query->bindParam(':so_nbr1', $soLine, PDO::PARAM_STR);
				$query->execute();
				$lineComments = $query->fetchAll(PDO::FETCH_ASSOC);

				
                foreach ($lineComments as $recent_comments_info_row) {
					if ($recent_comments_info_row['orderNum'] == $soLine) {
			...
			$obj = array(
				...
				"woDetails" => $woDetails,
				"ship" => $ship,
				"lastShipDate" => count($ship) ? $ship[count($ship) - 1] : null,
				"address" => $address,
				"orderFound" => $orderFound,
				"woOverall" => $woTotal > 0 ? ($woIssue / $woTotal) * 100 : "0.00",
				"lineOverall" => $shipped > 0 ? ($shipped / $ordered) * 100 : "0.00",
