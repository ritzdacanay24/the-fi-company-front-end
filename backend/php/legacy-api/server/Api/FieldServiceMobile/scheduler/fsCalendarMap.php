<?php
    ...
    try {  
        ...
        a.service_type,
        techId, 
        b.createdDate ticketCreatedDate, 
        b.dateSubmitted
    from fs_scheduler a
    ...
    LEFT JOIN (
        SELECT group_concat(user  ORDER BY user ASC SEPARATOR ', ') techs, group_concat(user_id SEPARATOR ', ') techId
        , fs_det_id 
        FROM fs_team GROUP BY fs_det_id 
    ) cc ON cc.fs_det_id = a.id
    where  a.active = 1 and date(request_date) between :dateFrom and :dateTo
    order by request_date asc
        ...
        echo $db_connect->json_encode($events);
