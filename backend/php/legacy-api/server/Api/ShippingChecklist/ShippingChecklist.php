...
namespace EyefiDb\Api\ShippingChecklist;
...
class ShippingChecklist
{
    ...
    public function __construct($db, $dbQad)
    {
        ...
        $this->uploader = new Upload($this->db1);
    }

    public function shippingChecklistReport()
    {
        $qry = "
            SELECT a.*, concat(b.first, ' ', b.last) created_by_name
        ...
        return (object) array(
            'email' => (object) array(
                'allowEmailSend' => array(
                    'Ritz Dacanay', 'Christine Martin', 'Priscilla Vargas'
                ),
                'addAddress' => array(
                    'christine.martin@the-fi-company.com', 'priscilla.vargas@the-fi-company.com'
                ),
                'bcAddress' => array(
    ...
    public function insert($data)
    {
        try {
            $qry = "
                INSERT INTO forms.shipping_checklist (
                    so 
                    , line
                    , pallets
                    , boxes
            ...
            $query->bindParam(":pallets", $data['pallets'], PDO::PARAM_INT);
            $query->bindParam(":boxes", $data['boxes'], PDO::PARAM_INT);
            $query->bindParam(":shipping_qty", $data['shipping_qty'], PDO::PARAM_INT);
            $query->bindParam(":data", $data['data'], PDO::PARAM_STR);
            $query->bindParam(":created_by", $data['created_by'], PDO::PARAM_INT);
            $query->bindParam(":created_date", $data['created_date'], PDO::PARAM_STR);
            $query->bindParam(":submitted_date", $data['submitted_date'], PDO::PARAM_STR);
            $query->bindParam(":part_number", $data['part_number'], PDO::PARAM_STR);
            $query->execute();
    ...
    public function getAttachments($id)
    {
        ...
        try {
            return  $this->uploader->getAttachments($id, 'Shipping Checklist', 'shippingChecklist');
        } catch (PDOException $e) {
            http_response_code(500);
            die($e->getMessage());
        }
    ...
    public function getBySoAndLine($soNumber, $lineNumber)
    {
        ...
        try {
            $qry = "
                ...
                FROM forms.shipping_checklist a
                LEFT JOIN db.users b ON b.id = a.created_by
                WHERE a.so = :so and a.line = :line
            ";
            $query = $this->db1->prepare($qry);
            $query->bindParam(":so", $soNumber, PDO::PARAM_STR);
            $query->bindParam(":line", $lineNumber, PDO::PARAM_INT);
            $query->execute();
            return $query->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            http_response_code(500);
    ...
    public function checkIfSalesOrderIsValid($salesOrderNumber)
    {
        ...
        try {
            ...
            $query->bindParam(":sod_nbr", $salesOrderNumber, PDO::PARAM_STR);
            $query->execute();
    ...
    public function updateById($data)
    {
        try {
            $qry = "
                UPDATE forms.shipping_checklist 
                SET so = :so
                    , line = :line
                    , pallets = :pallets
                    , boxes = :boxes
