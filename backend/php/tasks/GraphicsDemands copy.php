...
class GraphicsDemands
{
            ...
            if (($showOnlyLevel === null || $showOnlyLevel === 0) && isset($graphicsPartsLookup[$soPartNumber])) {
                ...
                if (!isset($seenComponents[$uniqueKey])) {
                    ...
                    $result[] = [
                        ...
                        'item_status' => isset($allPartsLookup[$soPartNumber]) ? $allPartsLookup[$soPartNumber]['status'] : ($graphicsPartsLookup[$soPartNumber]['status'] ?? ''),
    ...
    public function convertToTable($results)
    {
        ...
        $html .= '<th width="7%">SO Part</th>';
        $html .= '<th width="6%">Due Date</th>';
        $html .= '<th width="4%">SO Qty</th>';
        $html .= '<th width="7%">Item</th>';
        $html .= '<th width="9%">Item Description</th>';
        $html .= '<th width="7%">Graphics Part</th>';
        $html .= '<th width="9%">Graphics Description</th>';
        $html .= '<th width="3%">Status</th>';
        $html .= '<th width="4%">PM Code</th>';
        $html .= '<th width="5%">Qty Needed</th>';
        $html .= '<th width="3%">Level</th>';
        $html .= '<th width="7%">Parent</th>';
        $html .= '<th width="7%">BOM Path</th>';
        ...
        foreach ($groupedResults as $salesOrder => $lines) {
            ...
            $html .= '<td colspan="16"><strong><i class="fas fa-chevron-down"></i> Sales Order: ' . htmlspecialchars($salesOrder) . '</strong> <span class="badge bg-secondary">' . count($lines) . ' lines, ' . $totalSoItems . ' graphics items</span></td>';
            $html .= '</tr>';
            ...
            foreach ($lines as $lineNumber => $bomLevels) {
                ...
                $html .= '<td colspan="16"><em>&nbsp;&nbsp;<i class="fas fa-caret-right"></i> Line ' . htmlspecialchars($lineNumber) . '</em> <span class="badge bg-info">' . $totalLineItems . ' graphics items</span></td>';
                $html .= '</tr>';
                ...
                foreach ($bomLevels as $level => $levelRows) {
                    foreach ($levelRows as $row) {
                        ...
                        // Calculate indentation based on BOM path depth for better visual hierarchy
                        $pathParts = explode(' > ', $row['bom_path'] ?? '');
                        $indentLevel = max(0, count($pathParts) - 1);
                        $levelIndicator = $level == 0 ? '<i class="fas fa-star text-warning"></i>' : str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;', $indentLevel) . '<i class="fas fa-arrow-right text-muted"></i>';
    ...
    private function generateDebugTables($debugInfo, $allBomDataCount, $graphicsPartsCount, $soPartsCount, $bomDataSample, $graphicsPartsSample, $currentLevelParts, $bomRootParts, $specialDebug = null)
    {
        ...
        // Extract debug variables properly for display
        $incorrectBomItems = [];
        $unexpectedItems = [];
        
        if (isset($results['INCORRECT_BOM_ITEMS'])) {
            $incorrectBomItems = $results['INCORRECT_BOM_ITEMS'];
        }
        if (isset($results['UNEXPECTED_ITEMS'])) {
            $unexpectedItems = $results['UNEXPECTED_ITEMS'];
        }
        
        if (!empty($incorrectBomItems)) {
            $html .= '<div class="alert alert-danger">';
            $html .= '<h6>Incorrect BOM Items Found (Not in ERP BOM):</h6>';
            foreach ($incorrectBomItems as $item) {
                ...
                $html .= '<strong>Source:</strong> ' . htmlspecialchars($item['debug_source']) . ' | ';
                $html .= '<strong>BOM Path:</strong> ' . htmlspecialchars($item['bom_path']) . '</p>';
            }
            $html .= '</div>';
        }
        
        if (!empty($unexpectedItems)) {
    ...
    private function buildHierarchicalOrder($bomLevels)
    {
        ...
        // Recursive function to build hierarchical order
        ...
        // Group back by levels but maintain the hierarchical order
        $hierarchicalOrder = [];
        foreach ($hierarchicalItems as $item) {
            $level = $item['sort_level'];
            if (!isset($hierarchicalOrder[$level])) {
