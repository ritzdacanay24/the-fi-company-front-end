...
class MasterControl
{
    ...
    public function getLocations($items)
    {
        ...
        $qry = "
            select CAST(a.ld_loc AS CHAR(25)) ld_loc
                ...
                , c.loc_type
                , case when c.loc_type IN ('COI', 'FG', 'FS') THEN 1 ELSE 0 END format_location
                , case when c.loc_type IN ('COI', 'FG', 'FS') THEN LEFT(CAST(a.ld_loc AS CHAR(25)), 2) ELSE CAST(a.ld_loc AS CHAR(25)) END new_location_format
                , CAST(SUBSTRING(loc_loc, 1, 3) AS CHAR(25)) subbay
                , CAST(SUBSTRING(c.loc_loc, 1, 2) AS CHAR(25)) bay
            FROM ld_det a

            LEFT JOIN loc_mstr c ON c.loc_loc = CAST(a.ld_loc AS CHAR(25))
                AND loc_domain = 'EYE'
            ...
            WHERE a.ld_part IN ($in)
                ...
                order by CAST(SUBSTRING(loc_loc, 1, 3) AS CHAR(25))  DESC
            WITH (noLock)
        ";
        $query = $this->db->prepare($qry);
        $query->execute($items);
    ...
    public function getPickDetailsByWorkOrderNumber($pickNumber, $filteredSections)
    {
        ...
        if ($result) {
            foreach ($result as $row) {
                ...
                if ($row['ISHARDWARE'] == 1) {
                    ...
                    $row['first_location'] = $row['locations'] ? $row['locations'][0]['NEW_LOCATION_FORMAT'] : [];
                    $row['bay'] = $row['locations'][0]['BAY'];
                    $row['SUBBAY'] = $row['locations'][0]['SUBBAY'];
        ...
        //sort picklist
        function cmp($a, $b) {
            return strcmp($b->first_location, $a->first_location);
        }
        
        usort($details, function($a, $b) {
            if($b['bay'] == '01'){
                 strcmp($b['SUBBAY'], $a['SUBBAY']);
            }else{
...
$jsonText = json_encode($results['details']);
