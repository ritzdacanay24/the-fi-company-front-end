...
-- Step 1: Add Foreign Key Constraint (SKIPPED - Optional)
-- Foreign key constraint can cause issues if serial numbers don't exist yet
-- The triggers will handle validation automatically
-- Uncomment below if you want strict foreign key enforcement:
/*
...
-- Step 1b: Clean up empty strings and add UNIQUE constraint
-- First, convert empty strings to NULL (empty strings shouldn't be used for serial numbers)
UPDATE agsSerialGenerator SET serialNumber = NULL WHERE serialNumber = '' OR serialNumber IS NULL;

-- Now add UNIQUE constraint (NULL values are allowed and don't count as duplicates)
-- This prevents the same EyeFi serial from being assigned to multiple AGS records
ALTER TABLE agsSerialGenerator 
ADD CONSTRAINT unique_ags_eyefi_serial UNIQUE (serialNumber);

-- Step 2: Add Index for Performance (skip if already exists)
-- Note: Run this manually if index doesn't exist. Comment out if it causes errors.
-- ALTER TABLE agsSerialGenerator ADD INDEX idx_ags_eyefi_serial (serialNumber);
