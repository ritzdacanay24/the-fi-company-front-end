...
@Injectable()
export class VendorRepository extends BaseRepository<RowDataPacket> {
  ...
  constructor(@Inject(MysqlService) mysqlService: MysqlService) {
    super('eyefidb.fs_vendors', mysqlService);
  }
  ...
  async getAll(selectedViewType?: string): Promise<RowDataPacket[]> {
    if (selectedViewType === 'Active') {
      return this.rawQuery<RowDataPacket>(
        `SELECT * FROM eyefidb.fs_vendors WHERE active = 1`,
      );
    }

    if (selectedViewType === 'Inactive') {
      return this.rawQuery<RowDataPacket>(
        `SELECT * FROM eyefidb.fs_vendors WHERE active = 0 OR active IS NULL`,
      );
    }
