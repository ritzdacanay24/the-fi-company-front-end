...
export class ScheduledJobsRunnerService {
  ...
  @Cron('0 16 * * 1-5', { name: 'scheduled-jobs.daily-report-insert', timeZone: SCHEDULED_JOBS_TIMEZONE })
  async runDailyReportInsert(): Promise<void> {
    await this.scheduledJobsService.runJobIfEnabled('daily-report-insert');
  }

  @Cron('0 14 * * 1-5', { name: 'scheduled-jobs.inspection-email', timeZone: SCHEDULED_JOBS_TIMEZONE })
  async runInspectionEmail(): Promise<void> {
    await this.scheduledJobsService.runJobIfEnabled('inspection-email');
  }

  @Cron('0 17 * * 1-5', { name: 'scheduled-jobs.fs-job-report-evening', timeZone: SCHEDULED_JOBS_TIMEZONE })
  async runFsJobReportEvening(): Promise<void> {
    await this.scheduledJobsService.runJobIfEnabled('fs-job-report-evening');
  }

  @Cron('0 6 * * 1-5', { name: 'scheduled-jobs.past-due-field-service-requests', timeZone: SCHEDULED_JOBS_TIMEZONE })
  ...
  @Cron('0 7 * * 1-5', { name: 'scheduled-jobs.lnw-delivery', timeZone: SCHEDULED_JOBS_TIMEZONE })
