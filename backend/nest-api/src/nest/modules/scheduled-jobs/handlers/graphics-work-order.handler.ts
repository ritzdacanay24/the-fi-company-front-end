    ...
    const sql = `
      ...
      LEFT JOIN (
        SELECT 
          ...
          wo_rel_date,
          CASE
            WHEN DAYOFWEEK(wo_due_date) IN (1) THEN wo_due_date - 2
            WHEN DAYOFWEEK(wo_due_date) IN (2, 3) THEN wo_due_date - 4
            WHEN DAYOFWEEK(wo_due_date) IN (4) THEN wo_due_date - 2
            ELSE wo_due_date - 2
          END AS wo_due_date,
          wo_part,
          wo_qty_ord,
