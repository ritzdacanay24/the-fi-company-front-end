...
export const envValidationSchema = Joi.object({
  ...
  QAD_DSN_DEV: Joi.string().optional(),
  QAD_USER_DEV: Joi.string().optional(),
  QAD_PASSWORD_DEV: Joi.string().optional(),
  QAD_DSN_TEST: Joi.string().optional(),
  QAD_USER_TEST: Joi.string().optional(),
  QAD_PASSWORD_TEST: Joi.string().optional(),
  QAD_DSN_PROD: Joi.string().optional(),
  QAD_USER_PROD: Joi.string().optional(),
  QAD_PASSWORD_PROD: Joi.string().optional(),
  ...
  MAIL_TRANSPORT: Joi.alternatives().conditional('NODE_ENV', {
    is: 'development',
    then: Joi.string().valid('smtp', 'sendmail', 'ses').default('smtp'),
    otherwise: Joi.string().valid('smtp', 'sendmail', 'ses').default('sendmail'),
  }),
  AWS_SES_REGION: Joi.string().trim().default('us-east-1'),
  AWS_SES_ACCESS_KEY_ID: Joi.string().trim().allow('').optional(),
  AWS_SES_SECRET_ACCESS_KEY: Joi.string().trim().allow('').optional(),
