...
Test-RequiredFile "backend\database\migrations\create_photo_checklist_tables.sql"
Test-RequiredFile "backend\igt_api\photo-checklist-config.php"
Test-RequiredFile "backend\php\config\database.php"
Write-Host "✓ All required files found" -ForegroundColor Green

Write-Host ""
Write-Host "Step 2: Setting up database schema..."
try {
    $sqlContent = Get-Content "backend\database\migrations\create_photo_checklist_tables.sql" -Raw
...
if (Test-Path "backend\igt_api\photo-checklist-config.php") {
...
if (-not (Test-Path "backend\php\uploads\photos")) {
    New-Item -ItemType Directory -Path "backend\php\uploads\photos" -Force | Out-Null
}
