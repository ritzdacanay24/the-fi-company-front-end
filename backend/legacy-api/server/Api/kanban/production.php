...
$mainQry = "
    ...
    left join (
        select count(orderNum) id, orderNum, count(id) hits
        from comments 
        where type = 'kanban' and active = 1
        group by orderNum
    ...
    LEFT JOIN (
	 	SELECT wo_nbr, kanban_id, MAX(start_time) wo_start_time, MAX(end_time) wo_end_time
		 FROM kanban_timer b
         group by wo_nbr, kanban_id
	 ) e ON e.wo_nbr = a.wo_nbr AND e.kanban_id = d.id
";

if($wo_nbr){
    $mainQry .= " where a.wo_nbr = '$wo_nbr' ";
}
...
foreach ($queues as &$queue_row) {
    ...
    $queue_row['show_data']  = explode(",", $queue_row['show_data']);
    $queue_row['user_roles']  = explode(",", $queue_row['user_roles']);
        ...
        foreach ($wod_routing_info as $wod_routing_info_row) {
            if($resultDetails_row['wo_nbr'] == $wod_routing_info_row['wr_nbr']){
                if($resultDetails_row['routing'] == $wod_routing_info_row['wr_op']){
                    ...
                    $resultDetails_row['routingInfo'] = $wod_routing_info_row;
                }
            }
        }
