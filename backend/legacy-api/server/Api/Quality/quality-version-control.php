...
class QualityVersionControlAPI {
    ...
    public function __construct() {
        $this->database = new DatabaseEyefi();
    ...
    public function getRevisions($documentId) {
        ...
        $stmt = $this->conn->prepare("
            SELECT r.*, concat(u.first, ' ', u.last) as created_by_name, concat(a.first, ' ', a.last) as approved_by_name
            FROM quality_revisions r
            LEFT JOIN db.users u ON r.created_by = u.id
            LEFT JOIN db.users a ON r.approved_by = a.id
            WHERE r.document_id = ? 
