...
try {
    ...
    $qry = "
        SELECT a.*
            ...
            , DATE_FORMAT(request_date, '%m/%d/%Y') request_date
            , customerAsset
            , team
            , total_on_team
            , e.fs_det_id
    ...
    if ($searchCustomer) {
        ...
        $qry .= " where request_date = :dateFrom ";
        $query = $conn->prepare($qry);
    ...
    $qry = "
        select workOrderId, name, sum(cost) cost
        from eyefidb.fs_workOrderTrip 
        where workOrderId IN ($in)
        group by name, workOrderId
    ...
    , (case when sum(totalHrs) > @maxHrs THEN sum(totalHrs)-@maxHrs-IFNUll(sum(brkhrs),0) ELSE 0 END)  total_overtime_from_total_hrs
    , workOrderId
    , start
    , startFormate
    , IFNUll(sum(brkhrs),0) totalBrkHrs
    , IFNUll(sum(totalHrs),0) - IFNUll(sum(brkhrs),0) totalHrs
from (
    select *
    ...
    from (
        select  workOrderId 
            ...
            , case when work_order_labor_type = 1 THEN 1 ELSE 0 END  isTravel
            , truncate((sum(total_break)+sum(total_break_project))/60,2) brkhrs 
        from (
            SELECT workOrderId
                , projectStart start 
                , projectFinish end
                , TIMESTAMPDIFF(MINUTE,projectStart,projectFinish) totalHours
                , IFNULL(TIMESTAMPDIFF(MINUTE,brStart,brEnd),0) total_break 
                , case when proj_type = 'Break' then TIMESTAMPDIFF(MINUTE,projectStart,projectFinish) else 0 END total_break_project
                , proj_type
                , work_order_labor_type
            FROM eyefidb.fs_workOrderProject
            LEFT JOIN eyefidb.fs_scheduler_settings b ON b.value = proj_type AND type IN ('Work Order Project Type')
            where workOrderId IN ($in)
            
            order by projectStart ASC
        ) a
    group by workOrderId
        , DATE_FORMAT(date(start), '%m/%d/%Y')
        , DATE_FORMAT(date(end), '%m/%d/%Y')
        , DATE_FORMAT(date(start), '%a %m/%d/%Y')
        , work_order_labor_type
    order by  DATE_FORMAT(date(start), '%m/%d/%Y')
        , DATE_FORMAT(date(end), '%m/%d/%Y')
    ) a
) a
...
group by workOrderId
    ...
    $markUpPercent_default = 30;
    $cost_hourly_rate_default = 0;
    $cost_overtime_rate_default = 1.50;
    ...
    foreach ($mainInfo as &$row) {
        ...
        $markUpPercent = $row['mark_up_percent'];

        $request_date = date_create($row['request_date']);
        ...
        $row['rates'] = (object) array(
            "cost_hourly_rate" => $cost_hourly_rate_default,
            "cost_overtime_rate" => $cost_overtime_rate_default,

            "billing_hourly_rate" => $row['ef_hourly_rate'],
            "billing_overtime_rate" => $row['ef_overtime_hourly_rate'],
        );

        $row['testDate'] = $request_date;
        $row['details'] = array();
        ...
        foreach ($results as &$row1) {
            if ($row['workOrderId'] == $row1['workOrderId']) {
                ...
                if ($row1['isWeekend'] && $turn_on_weekend_overtime == 'true') {
                    ...
                    //add total overtime
                    //if weekend and 
                    $row1['total_overtime_from_total_hrs'] = $row1['travel_over_time_hrs'] + $row1['install_overtime_hrs'] - $row1['totalBrkHrs'];
                ...
                //if overtime > 0 remove break times from overtime

                if($row['travel_install_hrs']->total_overtime_from_total_hrs > 0){
                    $row['travel_install_hrs']->total_overtime_from_total_hrs -= $row1['totalBrkHrs'];
         ...
         * Based on effect date changes
         */
        if ($request_date >= $rateChangeEffectiveDate) {
